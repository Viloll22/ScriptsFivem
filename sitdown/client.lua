...
Citizen.CreateThread(function()
	while true do
		...
		if distance < 1.5 then
			ESX.Game.Utils.DrawText3D(vector3(pos.x, pos.y, pos.z+0.5), '~r~(F)~w~ Sentarse '..'~y~(Zx2)~w~ Levantarse',0.5)
		end
	...
	local id = pos.x .. pos.y .. pos.z
		...
		if GetEntityHeading(object) <=181 then 
		 ...
		 if string.match(modelName, "chair") or string.match(modelName, "Chair") then
			...
			TaskStartScenarioAtPosition(playerPed, currentScenario, pos.x, pos.y, pos.z - data.verticalOffset-0.2, GetEntityHeading(object)+180.0, 0, true, true)
		 end
